...
<head>
    ...
    <style>
        ...
        body, html { margin: 0; padding: 0; width: 100%; height: 100%; overflow: hidden; background-color: var(--bg-dark); color: var(--text-main); font-family: ui-sans-serif, system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Arial, sans-serif; }
        
        /* Event grid pattern from Paralelo37 */
        .grid-pattern {
            position: absolute;
            inset: 0;
            pointer-events: none;
            background-image: linear-gradient(rgba(27,42,74, 0.4) 1px, transparent 1px), linear-gradient(90deg, rgba(27,42,74, 0.4) 1px, transparent 1px);
            background-size: 60px 60px;
            z-index: 1;
        }
        
        .slide { position: absolute; top: 0; left: 0; width: 100%; height: 100%; display: flex; flex-direction: column; justify-content: center; align-items: center; opacity: 0; visibility: hidden; transition: opacity 0.4s ease, transform 0.4s ease; transform: translateX(30px) scale(0.98); padding: 4rem; box-sizing: border-box; z-index: 2; }
        ...
        .progress-bar { position: fixed; top: 0; left: 0; height: 4px; background: linear-gradient(90deg, var(--primary), var(--secondary)); transition: width 0.3s ease; z-index: 50; box-shadow: 0 0 10px rgba(148,168,59,0.5); }
        .slide-counter { position: fixed; bottom: 1.5rem; right: 1.5rem; font-size: 1rem; font-weight: 600; font-family: 'Fira Code', monospace; color: var(--primary); z-index: 50; background: rgba(11, 17, 32, 0.8); padding: 0.5rem 1rem; border-radius: 9999px; border: 1px solid var(--primary-light); box-shadow: 0 0 10px rgba(148,168,59,0.2); backdrop-filter: blur(5px); }
        ...
        h3 { font-size: 1.75rem; font-weight: 600; margin-bottom: 1rem; color: var(--text-main); text-align: center; }
        ...
        /* Glassmorphism Cards */
        .card { background: rgba(27, 42, 74, 0.6); backdrop-filter: blur(10px); border: 1px solid rgba(148, 168, 59, 0.2); border-radius: 1rem; padding: 2.5rem; box-shadow: 0 20px 25px -5px rgba(0, 0, 0, 0.3), 0 0 20px rgba(148, 168, 59, 0.05); width: 100%; }
        
        /* Glow Cards for replacing emojis */
        .glow-card { display: inline-flex; justify-content: center; align-items: center; width: 4rem; height: 4rem; border-radius: 1rem; background: rgba(148, 168, 59, 0.1); border: 1px solid var(--primary); box-shadow: 0 0 15px rgba(148, 168, 59, 0.3); color: var(--primary); margin-bottom: 1rem; }
        ...
        .code-window { width: 100%; max-width: 1000px; border-radius: 0.75rem; overflow: hidden; box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.5), 0 0 20px rgba(148,168,59,0.1); border: 1px solid rgba(148, 163, 184, 0.2); }
        ...
        pre { background: #070B14; padding: 1.5rem; margin: 0; overflow-x: auto; font-family: 'Fira Code', monospace; font-size: 1.2rem; text-align: left; line-height: 1.6; }
        ...
        .highlight { color: var(--secondary); font-weight: 700; text-shadow: 0 0 8px rgba(229,169,60,0.5); }
        
        .badge { display: inline-block; padding: 0.5rem 1.5rem; background: rgba(148, 168, 59, 0.1); border: 1px solid var(--primary); border-radius: 9999px; font-size: 1.1rem; font-weight: 600; color: var(--primary); box-shadow: 0 0 10px rgba(148,168,59,0.2); }
        ...
        .diagram-box { fill: rgba(27,42,74,0.8); stroke: var(--primary); stroke-width: 2; rx: 12; filter: drop-shadow(0 0 10px rgba(148,168,59,0.4)); }
        ...
        .diagram-arrow { stroke: var(--secondary); stroke-width: 3; fill: none; marker-end: url(#arrowhead); stroke-dasharray: 8; animation: dash 20s linear infinite; filter: drop-shadow(0 0 5px rgba(229,169,60,0.5)); }
        .diagram-arrow-solid { stroke: var(--secondary); stroke-width: 3; fill: none; marker-end: url(#arrowhead); filter: drop-shadow(0 0 5px rgba(229,169,60,0.5)); }
        @keyframes dash { to { stroke-dashoffset: -1000; } }
        
        .pulse { animation: pulse 2s cubic-bezier(0.4, 0, 0.6, 1) infinite; }
        @keyframes pulse { 0%, 100% { opacity: 1; transform: scale(1); box-shadow: 0 0 15px rgba(148,168,59,0.4); } 50% { opacity: .8; transform: scale(1.02); box-shadow: 0 0 25px rgba(148,168,59,0.8); } }
        
        .img-glow { border-radius: 1rem; border: 1px solid rgba(148,168,59,0.3); }
    </style>
</head>
<body>
    <div class="grid-pattern"></div>
    <div class="progress-bar" id="progress"></div>
    <div class="slide-counter" id="counter">1 / 28</div>

    <svg width="0" height="0" style="position:absolute;">
        <defs>
            <marker id="arrowhead" markerWidth="10" markerHeight="7" refX="9" refY="3.5" orient="auto">
                <polygon points="0 0, 10 3.5, 0 7" fill="#E5A93C" />
            </marker>
        </defs>
    </svg>
    ...
    <!-- Slide 1 -->
    <div class="slide">
        <div class="badge mb-8 border-primary text-primary bg-primary/20 backdrop-blur-sm">Con el apoyo y hardware de Nitrokey</div>
        <h1>Adiós a las contraseñas: FIDO2 y WebAuthn,<br>del concepto al código (y a tu propia llave)</h1>
        <p class="text-2xl mt-8 text-primary font-mono font-bold tracking-widest bg-navy/50 p-4 rounded-xl shadow-lg border border-primary/20 backdrop-blur-sm">Paralelo 37 · XauenDevs · Jaén</p>
    </div>
    ...
    <div class="slide">
        ...
        <div class="grid-2">
            <div class="flex justify-center">
                <img src="fotomia.jpeg" class="w-64 h-64 rounded-full border-4 border-primary drop-shadow-[0_0_20px_rgba(148,168,59,0.5)] object-cover shadow-2xl">
            </div>
            <div>
                <ul>
                ...
                <div class="mt-8 flex gap-6 text-secondary text-xl font-mono">
                    <span class="px-3 py-1 bg-secondary-dark/20 border border-secondary/50 rounded shadow-sm backdrop-blur-sm">github.com/xtoxico</span>
                    <span class="px-3 py-1 bg-secondary-dark/20 border border-secondary/50 rounded shadow-sm backdrop-blur-sm">normapro.es</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="slide">
        ...
        <div class="card w-full max-w-4xl text-left border-dashed duct-tape-container">
            ...
            <div class="border-b border-primary/20 pb-6 mb-6 mt-4">
                <h3 class="flex items-center gap-6 text-2xl !text-left">
    ...
    <div class="slide">
        ...
        <div class="grid-2">
            <div class="flex justify-center w-full">
                <img src="assets/images/alberto_password.jpg" alt="Alberto y su Password" class="img-glow drop-shadow-[0_20px_50px_rgba(229,169,60,0.2)] w-full h-auto object-cover aspect-video">
            </div>
            <div class="card">
                <p class="text-left text-xl text-text-main m-0"><strong>Correo:</strong> <span class="text-primary-light">albertthegreat@hotmail.com</span></p>
                <p class="text-left text-sm text-text-muted mb-6">(creado en 2004)</p>
                <p class="text-left text-xl text-text-main m-0"><strong>Contraseña maestra:</strong> <span class="font-mono text-danger font-bold tracking-widest bg-danger/10 px-2 py-1 rounded">P4$$w0rd</span></p>
                <div class="mt-8 p-6 bg-bg-dark rounded-lg border border-primary/30 relative overflow-hidden">
                    ...
                    <p class="text-lg italic text-text-main text-left">"Lleva una mayúscula, un cuatro, dos dólares y un cero. Es criptografía cuántica."</p>
    ...
    <div class="slide">
        ...
        <div class="relative w-full max-w-5xl">
            ...
            <div class="card bg-bg-dark/80 border-danger z-10 relative flex flex-col items-center py-12 backdrop-blur-md">
                ...
                <p class="text-2xl mb-8 font-mono text-text-main text-center">haveibeenpwned.com</p>
    ...
    <div class="slide">
        ...
        <div class="grid grid-cols-2 gap-6 w-full max-w-5xl duct-tape-container">
            ...
            <div class="card bg-bg-dark border-primary/30">
                <h3 class="text-xl text-primary !text-left">Complejidad absurda</h3>
                <p class="text-lg text-left m-0">Mayúsculas, minúsculas, números, caracteres raros y el grupo sanguíneo.</p>
            </div>
            <div class="card bg-bg-dark border-primary/30">
                <h3 class="text-xl text-primary !text-left">Caducidad</h3>
            ...
            <div class="card bg-bg-dark border-primary/30">
                <h3 class="text-xl text-primary !text-left">SMS 2FA</h3>
                <p class="text-lg text-left m-0">SIM swapping te saluda con la mano.</p>
            </div>
            <div class="card bg-bg-dark border-primary/30">
                <h3 class="text-xl text-primary !text-left">Apps autenticadoras</h3>
    ...
    <div class="slide">
        ...
        <div class="grid-2 mt-8">
            <div class="card text-center border-danger bg-danger/10">
                <h3 class="text-danger">Tradicional</h3>
                <div class="text-6xl my-8 font-mono">🧑 ↔️ 🔑 ↔️ 🖥️</div>
                <p class="text-lg text-text-main m-0">Usuario y servidor comparten el <strong class="text-danger">mismo secreto</strong>.<br>Si el servidor cae, caen todos.</p>
            </div>
            <div class="card text-center border-primary bg-primary/10 pulse">
                <h3 class="text-primary-light">FIDO2</h3>
                <div class="text-6xl my-8 font-mono">🧑[🗝️] ↔️ 🔒[🖥️]</div>
                <p class="text-lg text-text-main m-0">El usuario tiene la <strong class="text-primary">llave privada</strong> en su hardware.<br>El servidor solo tiene la cerradura (<strong class="text-secondary">clave pública</strong>).</p>
            </div>
        </div>
    </div>
    ...
    <div class="slide">
        ...
        <div class="flex flex-col items-center gap-6 w-full max-w-4xl">
            <div class="card w-full border-danger bg-danger/10 p-6 flex justify-between items-center">
            ...
            <div class="text-xl text-secondary font-mono py-2 font-bold drop-shadow-[0_0_8px_rgba(229,169,60,0.8)]">⬇️ Navegador inyecta ORIGEN en la firma ⬇️</div>
            
            <div class="card w-full border-primary bg-primary/10 p-6 pulse">
    ...
    <div class="slide">
        ...
        <svg viewBox="0 0 900 350" class="w-full max-w-5xl bg-bg-dark/80 rounded-xl p-4 border border-primary/20 backdrop-blur-md">
            ...
            <rect x="670" y="160" width="160" height="40" fill="var(--primary)" rx="5" class="pulse" style="filter: drop-shadow(0 0 10px rgba(148,168,59,0.8));"/>
    ...
    <div class="slide">
        ...
        <div class="flex justify-center items-center gap-12 w-full max-w-4xl bg-bg-dark/80 p-12 rounded-2xl border border-primary/30 duct-tape-container backdrop-blur-md">
            ...
            <div class="text-5xl text-primary font-bold">+</div>
            <div class="text-center card border-primary bg-primary/20 p-6 m-0 w-auto shadow-[0_0_30px_rgba(148,168,59,0.3)]">
...
<span class="comment">// ¡Peligro! Hardware clonado o petición repetida.</span></code></pre>
    ...
    <div class="slide">
        ...
        <div class="card w-full max-w-4xl border-primary bg-primary/10 shadow-[0_0_30px_rgba(148,168,59,0.15)]">
            ...
            <div class="mt-8 p-6 bg-bg-dark rounded border border-primary/30 font-mono text-primary-light relative overflow-hidden shadow-inner">
  ...
  <span class="keyword">async</span> <span class="function">startRegistration</span>(username: <span class="keyword">string</span>) {
    ...
    <div class="slide">
        ...
        <div class="grid-2">
            ...
            <div class="card bg-bg-dark border-secondary/30 p-8 h-full shadow-[0_0_30px_rgba(229,169,60,0.15)]">
                <h3 class="text-secondary border-b border-secondary/20 pb-4">Roaming (Externos)</h3>
                <div class="my-6 flex gap-4 justify-center">
                    <div class="glow-card" style="border-color:var(--secondary);color:var(--secondary);box-shadow:0 0 15px rgba(229,169,60,0.3);background:rgba(229,169,60,0.1);"><svg class="w-8 h-8"><use href="#icon-key"></use></svg></div>
                    <div class="glow-card" style="border-color:var(--secondary);color:var(--secondary);box-shadow:0 0 15px rgba(229,169,60,0.3);background:rgba(229,169,60,0.1);"><svg class="w-8 h-8"><use href="#icon-plug"></use></svg></div>
                </div>
                <ul class="text-lg">
    ...
    <!-- Slide 18: Passkeys Modernos -->
    <div class="slide">
        <h2>El presente: Passkeys en tu bolsillo</h2>
        <h3 class="mb-10 text-primary-light font-mono">FIDO2 disfrazado de comodidad absoluta</h3>
        <div class="grid grid-cols-3 gap-8 w-full max-w-6xl">
            <div class="card bg-bg-dark border-primary/30 flex flex-col items-center p-8">
                <div class="text-6xl mb-6">🪟</div>
                <h3 class="text-2xl text-text-main mb-4">Windows Hello</h3>
                <p class="text-lg text-text-muted text-center m-0">Inicia sesión con tu rostro o huella. El TPM de tu PC es el hardware criptográfico.</p>
            </div>
            <div class="card bg-bg-dark border-primary/30 flex flex-col items-center p-8">
                <div class="text-6xl mb-6">🍎</div>
                <h3 class="text-2xl text-text-main mb-4">Ecosistema Apple</h3>
                <p class="text-lg text-text-muted text-center m-0">FaceID / TouchID. Las claves se sincronizan de forma encriptada vía iCloud Keychain.</p>
            </div>
            <div class="card bg-bg-dark border-primary/30 flex flex-col items-center p-8">
                <div class="text-6xl mb-6">🤖</div>
                <h3 class="text-2xl text-text-main mb-4">Android Passkeys</h3>
                <p class="text-lg text-text-muted text-center m-0">Huella digital en el móvil. Respaldado por Google Password Manager y el chip Titan.</p>
            </div>
        </div>
        <div class="mt-12 p-6 bg-primary/20 border border-primary rounded-xl pulse max-w-4xl text-center">
            <p class="text-xl font-bold m-0 text-text-main">Ya estás usando criptografía asimétrica FIDO2 en tu día a día sin saberlo.</p>
        </div>
    </div>
    ...
    <div class="slide">
        ...
        <div class="bg-white/95 px-8 py-4 rounded-2xl mb-12 shadow-[0_0_30px_rgba(148,168,59,0.3)] inline-block">
             <img src="Nitrokey_Logo.svg" alt="Nitrokey Logo" class="h-10 w-auto" />
        </div>
        <div class="grid-2">
            <div class="flex flex-col items-center justify-center w-full">
                <img src="assets/images/nitrokey_3a.jpg" alt="Nitrokey 3A" class="img-glow drop-shadow-[0_20px_50px_rgba(148,168,59,0.4)] w-full h-auto aspect-video object-cover mb-6">
    ...
    <div class="slide">
        ...
        <div class="card text-center py-20 px-8 border-primary bg-primary/10 max-w-4xl shadow-[0_0_50px_rgba(148,168,59,0.15)]">
            ...
            <p class="text-xl text-text-muted font-mono m-0">Autenticado en milisegundos sin escribir una sola tecla.</p>
        </div>
    </div>
    ...
    <div class="slide">
        ...
        <div class="flex justify-center w-full max-w-4xl">
            <img src="assets/images/raspberry_pi_pico.jpg" alt="Raspberry Pi Pico 2" class="img-glow drop-shadow-[0_20px_50px_rgba(148,168,59,0.4)] w-full h-auto aspect-video object-cover">
        </div>
    </div>
    ...
    <div class="slide">
        ...
        <div class="grid-2">
            <div class="card border-danger/50 bg-danger/10 h-full">
            ...
            <div class="card border-primary bg-primary/10 h-full shadow-[0_0_30px_rgba(148,168,59,0.2)]">
    ...
    <div class="slide">
        ...
        <div class="card w-full max-w-4xl p-10 border-secondary bg-secondary/10">
    ...
    <!-- Slide 24: Nitro vs PicoFIDO -->
    <div class="slide">
        <h2>¿Nitrokey 3A o Pico FIDO?</h2>
        <h3 class="mb-10 font-mono text-text-muted">Por qué el silicio casero no sustituye a la ingeniería de seguridad</h3>
        <div class="grid-2">
            <div class="card border-secondary/50 bg-secondary/10 h-full flex flex-col justify-start">
                <h3 class="text-secondary text-center text-3xl font-mono mb-6 border-b border-secondary/20 pb-4 flex items-center justify-center gap-3"><svg class="w-8 h-8"><use href="#icon-wrench"></use></svg> Pico FIDO</h3>
                <ul class="text-lg space-y-5">
                    <li><strong class="text-secondary-dark">Educativo:</strong> Perfecto para aprender cómo funciona el protocolo por dentro.</li>
                    <li><strong class="text-secondary-dark">Silicio General:</strong> El RP2350 es genial, pero no es un chip criptográfico especializado.</li>
                    <li><strong class="text-secondary-dark">Construcción:</strong> Placa desnuda. Se te cae en el café y se acabó el login.</li>
                    <li><strong class="text-secondary-dark">Attestation:</strong> Al ser hardware no certificado, algunos servicios bancarios o corporativos pueden rechazarlo.</li>
                </ul>
            </div>
            <div class="card border-primary bg-primary/10 h-full shadow-[0_0_30px_rgba(148,168,59,0.2)] flex flex-col justify-start">
                <h3 class="text-primary-light text-center text-3xl font-mono mb-6 border-b border-primary/20 pb-4 flex items-center justify-center gap-3"><svg class="w-8 h-8"><use href="#icon-key"></use></svg> Nitrokey 3A</h3>
                <ul class="text-lg space-y-5">
                    <li><strong class="text-primary">Secure Element:</strong> Hardware diseñado 100% para evitar extracción de claves (ni lijando el chip).</li>
                    <li><strong class="text-primary">Certificaciones:</strong> Confianza total. FIDO Alliance y estándares empresariales.</li>
                    <li><strong class="text-primary">Durabilidad:</strong> Sellada con resina epoxi. A prueba de agua, polvo y vida real.</li>
                    <li><strong class="text-primary">Producción:</strong> La herramienta fiable que usas para asegurar tu nómina, tus servidores y tus secretos de empresa.</li>
                </ul>
            </div>
        </div>
    </div>
    ...
    <div class="slide">
        ...
        <div class="card w-full max-w-4xl border-secondary bg-secondary/10 p-10 shadow-[0_0_40px_rgba(229,169,60,0.15)]">
            ...
            <ul class="text-xl space-y-6">
                ...
                <li><strong>Attestation:</strong> La capacidad del backend de exigir llaves certificadas.</li>
            </ul>
        </div>
    </div>
    ...
    <div class="slide">
        ...
        <div class="grid grid-cols-3 gap-8 w-full max-w-6xl">
            <div class="card text-center p-8 border-danger/30 bg-danger/10 h-full flex flex-col items-center justify-start">
    ...
    <div class="slide">
        ...
        <div class="grid-2 max-w-5xl">
            <div class="flex flex-col items-center">
                <div class="bg-white p-6 rounded-2xl w-64 h-64 flex justify-center items-center mb-6 shadow-[0_0_30px_rgba(148,168,59,0.3)]">
            ...
            <div class="w-full">
                ...
                <div class="flex flex-wrap gap-4 mt-8">
                    ...
                    <span class="badge border-primary text-primary-light bg-primary/20">XauenDevs</span>
                    <span class="badge border-secondary text-secondary" style="box-shadow: 0 0 10px rgba(229,169,60,0.2); background: rgba(229,169,60,0.1);">Nitrokey</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <!-- Slide 28 -->
    <div class="slide">
        <div class="card border-primary/50 bg-bg-dark/80 p-16 mb-12 max-w-5xl shadow-[0_0_50px_rgba(148,168,59,0.15)] relative overflow-hidden backdrop-blur-md">
        ...
        <div class="text-center mt-8">
            <h3 class="text-3xl mb-4 text-text-main !text-center">Antonio Tirado Peña</h3>
            <p class="text-xl text-text-muted font-mono m-0">yosoy@antoniotirado.com · @xtoxico · normapro.es</p>
            <div class="mt-12 p-4 bg-primary/20 border border-primary rounded-xl inline-block pulse backdrop-blur-sm">
...
print("Updated index.html to v4 successfully.")
