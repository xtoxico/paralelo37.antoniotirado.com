...
type Usuario = { reto: string; nombre?: string; apellidos?: string; credenciales: Credencial[] };
...
app.post('/auth/register/begin', async (peticion, respuesta) => {
  const email: string = peticion.body.email;
  const nombre: string = peticion.body.nombre;
  const apellidos: string = peticion.body.apellidos;
  
  const bd = leerBD();
  const registro = ficha(bd, email);
  ...
  const opciones = await generateRegistrationOptions({
    ...
    excludeCredentials: registro.credenciales.map((c) => ({ id: c.id, transports: c.transports as any })),
    authenticatorSelection: {
      residentKey: 'required', // 'required' = Obligamos a guardar la clave dentro (passkey discoverable).
...
app.post('/auth/login/begin', async (_peticion, respuesta) => {
  // Ya no pedimos usuario. El navegador buscará en su llavero las credenciales 
  // asociadas a nuestro rpID y nos dirá quién se está logueando.
  const opciones = await generateAuthenticationOptions({
    rpID,
    // Al omitir 'allowCredentials', forzamos el modo "Discoverable Credentials" (Resident Keys).
    // El navegador preguntará al usuario "¿Con qué cuenta quieres entrar?".
...
app.post('/auth/login/complete', async (peticion, respuesta) => {
  ...
  // Como no pedimos el email al principio, tenemos que buscar de quién es esta credencial.
  ...
  const { verified, authenticationInfo } = await verifyAuthenticationResponse({
    response: firma,
    expectedChallenge: retoGlobalLogin,
    expectedOrigin: origin,
    expectedRPID: rpID,
    // Le pasamos la clave pública guardada, devolviéndola de texto a bytes.
    credential: {
      id: credencialEncontrada.id,
      publicKey: Uint8Array.from(Buffer.from(credencialEncontrada.publicKey, 'base64url')),
      counter: credencialEncontrada.counter,
      transports: credencialEncontrada.transports as any,
    },
  ...
  credencialEncontrada.counter = authenticationInfo.newCounter;
  guardarBD(bd);

  // Aquí es donde, en una aplicación real, emitirías la cookie de sesión o el JWT.
  respuesta.json({ verificado: true, usuario: usuarioEncontrado });
});
